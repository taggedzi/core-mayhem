...
export function spawnAmmo(side:Side){
  const {W,H} = sim; const x = side===Side.LEFT? 30+Math.random()*40 : W-30-Math.random()*40; const y=H*0.92+Math.random()*10;
  const pool = Math.random()<0.7? ['basic','heavy'] : ['basic','heavy','volatile','emp','repair','shield'];
...
export function beforeUpdateAmmo(){
  ...
  for(const b of bodies){
    const plug=(b as any).plugin; if(!plug) continue;
    if(plug.kind==='ammo'){
      ...
      plug.age=(plug.age||0)+dt; plug.idle = speed<0.15 ? (plug.idle||0)+dt : 0;
      ...
      if(b.position.x<-120||b.position.x>sim.W+120||b.position.y<-120||b.position.y>sim.H+20 || plug.idle>8){
        World.remove(sim.world,b); (plug.side===Side.LEFT? sim.ammoL--: sim.ammoR--);
      }
    }
  }
}