...
export type PinField = { mid:number; width:number };
...
export function makePins(side: -1|1, opts?: { anchor?: number; gap?: number }): PinField {
  ...
  const mid =
    opts?.anchor != null
      ? (side < 0 ? (opts.anchor + gap + width/2) : (opts.anchor - gap - width/2))
      : (side < 0 ? W*0.18 : W*0.82);
  ...
  for(let r=0;r<rows;r++){
    if((r+1)%3===0){
      ...
      for(let c=0;c<cols;c++){
        ...
        const y = startY + r*sy;
        const poly = Bodies.polygon(x,y,(r%2?3:4),rad,{friction:0,restitution:0.12,frictionAir:0.02});
        (poly as any).plugin = {kind:'rotor', spinDir: Math.random()<.5?-1:1};
        World.add(sim.world, [poly, Constraint.create({pointA:{x,y}, bodyB:poly, length:0, stiffness:1})]);
      ...
      for(let c=0;c<cols;c++){
        ...
        const y = startY + r*sy;
        const p = Bodies.circle(x,y,4,{isStatic:true,restitution:.9});
