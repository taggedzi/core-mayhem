...
export function attachCollisions(){
  Events.on(sim.engine,'collisionStart', e=>{
    for(const p of e.pairs){
      ...
      else if(b.kind==='projectile'&&(a.kind==='coreRing'||a.kind==='coreCenter')) hit(B,A);
    }
  });
}
...
function deposit(ammo:any, container:any){
  ...
  if(ammo.plugin.side===Side.LEFT) sim.ammoL--; else sim.ammoR--;
}
...
function hit(proj:any, coreBody:any){
  ...
  else { const sp=angleToSeg(core, proj.position); const d0=Math.round(dmg*sp.w0), d1=dmg-d0; core.segHP[sp.i0]=Math.max(0,core.segHP[sp.i0]-d0); core.segHP[sp.i1]=Math.max(0,core.segHP[sp.i1]-d1); }
